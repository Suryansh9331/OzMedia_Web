...
const Header = () => {
  ...
  return (
    <>
      {/* Blur Overlay */}
      <div 
        className={`fixed inset-0 z-30 transition-all duration-500 ease-in-out ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
        style={{
          background: 'rgba(0, 0, 0, 0.3)',
          backdropFilter: 'blur(8px)',
          WebkitBackdropFilter: 'blur(8px)'
      ...
      {/* Navbar */}
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-in-out ${isScrolled ? 'bg-[#c4bec7] shadow-lg mx-4 mt-4 rounded-full backdrop-blur-md' : 'bg-[#c4bec7] mx-0 mt-0 rounded-none'}`}
        style={{ backgroundColor: '#c4bec7', boxShadow: isScrolled ? '0 10px 30px rgba(0,0,0,0.1)' : 'none' }}
      >
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex-shrink-0">
      ...
      {/* Mega Dropdown */}
      <div 
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-700 ease-out ${isMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible'} md:block`}
        style={{
          marginTop: isScrolled ? '5.5rem' : '4.5rem',
          marginLeft: isScrolled ? '1rem' : '0',
          marginRight: isScrolled ? '1rem' : '0',
      ...
      >
        <div 
          className={`relative max-w-5xl mx-auto rounded-2xl shadow-2xl backdrop-blur-xl ${isMenuOpen ? 'animate-unroll' : 'animate-roll-up'}`}
          style={{
            background: 'linear-gradient(135deg, rgba(196, 190, 199, 0.95) 0%, rgba(212, 205, 215, 0.95) 50%, rgba(196, 190, 199, 0.95) 100%)',
            boxShadow: '0 25px 50px rgba(0,0,0,0.25), 0 0 0 1px rgba(255,255,255,0.1), inset 0 1px 0 rgba(255,255,255,0.2)',
            border: '1px solid rgba(255,255,255,0.2)',
